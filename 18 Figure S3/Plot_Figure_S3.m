...
h_fig23 = figure(23);
set(h_fig23,'color','w','Position',[100 200 800 600]) 
%%

N_looms = 8;
t = (1:80000)/20000;
load ../'09 Loom'/r_burst_data.mat

% Remove artifacts at the start and end of traces from filtering
...
for nl = 1:N_looms/2
    ...
    if (nl == 1)
        ...
        legend boxoff
        text(-1.15,25,'B1','fontweight','bold','fontsize',14)
    elseif (nl == 2)
        text(-1.15,25,'B2','fontweight','bold','fontsize',14)
    elseif (nl == 3)
        text(-1.15,25,'B3','fontweight','bold','fontsize',14)
    elseif (nl == 4)
        text(-1.15,25,'B4','fontweight','bold','fontsize',14)
    end
end
for nl = 1+N_looms/2:N_looms
    ...
    ylabel('Spike rate (Hz)')
    if (nl == 5)
        text(-1.15,25,'C1','fontweight','bold','fontsize',14)
    elseif (nl == 6)
        text(-1.15,25,'C2','fontweight','bold','fontsize',14)
    elseif (nl == 7)
        text(-1.15,25,'C3','fontweight','bold','fontsize',14)
    elseif (nl == 8)
        text(-1.15,25,'C4','fontweight','bold','fontsize',14)
    end
end
...
for nl = 1:N_looms
    ...
    ylabel('Angle')
    if (nl < 5)
        text(0.5,80,['r / v = ' num2str(round(r_over_v(nl))) ' (ms)'])
    else
        text(0.5,80,['r / v = ' num2str(round(r_over_v(nl-4))) ' (ms)'])
    end
    %     legend('3', '15', '500','location','northwest')
    %     legend boxoff
    if (nl == 1)
        text(-1.1,125,'A1','fontweight','bold','fontsize',14)
    elseif (nl == 2)
        text(-1.1,125,'A2','fontweight','bold','fontsize',14)
    elseif (nl == 3)
        text(-1.1,125,'A3','fontweight','bold','fontsize',14)
    elseif (nl == 4)
        text(-1.1,125,'A4','fontweight','bold','fontsize',14)
    end
end
...
subplot(ny,nx,[57 58 59 65 66 67 73 74 75])
hold off
...
legend boxoff
text(6,25,{'Mean spike' 'rate 3-3.5 s'})
set(gca,'xtick',[5:10 20:10:100 200:100:1000 2000 3000],'xticklabel',{'' '' '' '' '' '10' '' '' '' '' '' '' '' '' '100' '' '' '' '' '' '' '' '' '1000' '' ''})
text(1.5,32,'D1','fontweight','bold','fontsize',14)

subplot(ny,nx,[60 61 62 68 69 70 76 77 78])
hold off
semilogx(r_over_v,mn_mrstage4_3(1:4),'-^','color',bucol(3,:),'markerfacecolor',bucol(3,:))
hold on
semilogx(r_over_v,mn_mrstage4_1(1:4),'-o','color',bucol(1,:),'markerfacecolor',bucol(1,:))
...
xlabel('r/v (ms)')
ylabel('Spike rate (Hz)')
text(6,25,{'Mean spike' 'rate 3.5-4 s'})
set(gca,'xtick',[5:10 20:10:100 200:100:1000 2000 3000],'xticklabel',{'' '' '' '' '' '10' '' '' '' '' '' '' '' '' '100' '' '' '' '' '' '' '' '' '1000' '' ''})
text(1.5,32,'D2','fontweight','bold','fontsize',14)
...
N_cells = 6;
N_directions = 9;
flicker_freq = [1 2 4 8 10 15 25 50 100];

load ../'08 Flicker'/r_burst_data.mat

mnrburst_1 = mean(r_burst_1,3);
mnrburst_3 = mean(r_burst_3,3);
mn_mrburst_1 = mean(mnrburst_1,1);
se_mrburst_1 =  std(mnrburst_1,1)/sqrt(N_cells);
mn_mrburst_3 = mean(mnrburst_3,1);
se_mrburst_3 =  std(mnrburst_3,1)/sqrt(N_cells);

subplot(ny,nx,[63 64 71 72 79 80])
hold off
semilogx(flicker_freq, mn_mrburst_3,'-^','color',bucol(3,:),'markerfacecolor',bucol(3,:))
hold on
semilogx(flicker_freq, mn_mrburst_1,'-o','color',bucol(1,:),'markerfacecolor',bucol(1,:))
errorbar(flicker_freq, mn_mrburst_3,se_mrburst_3,'-^','color',bucol(3,:),'markerfacecolor',bucol(3,:))
errorbar(flicker_freq, mn_mrburst_1,se_mrburst_1,'-o','color',bucol(1,:),'markerfacecolor',bucol(1,:))
...
ylabel('Spike rate (Hz)')
text(1,25,'Flicker')
text(0.25,32,'E','fontweight','bold','fontsize',14)
set(gca,'xtick',[0.8:0.1:1 2:10 20:10:100],'xticklabel',{'' '' '1' '' '' '' '' '' '' '' '' '10' '' '' '' '' '' '' '' '' '100'})
